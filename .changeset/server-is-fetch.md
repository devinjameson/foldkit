...
**Migration:** drop `ssr.build.entry`. Keep `ssr.serverEntry` as `renderPage`. Start the app with `node scripts/serve.ts` instead of `node dist/server/main.js`. A Cloudflare Worker can default-export the same `fetch.js` module. The handler uses the platform `Request.url` unless `ssr.origin` or `ORIGIN` is set.
