...
**Migration:** drop `ssr.build.entry`. Keep `ssr.serverEntry` as `renderPage`. Start the app with `node scripts/serve.ts` instead of `node dist/server/main.js`. A Cloudflare Worker can default-export the same `fetch.js` module. The handler trusts `Request.url` as the platform constructed it; a Node adapter resolves the raw request target against its configured origin before calling `fetch`, as `scripts/serve.ts` does.
