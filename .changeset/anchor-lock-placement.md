...
Add `lockPlacement` to `AnchorConfig`. It keeps the side that an anchored panel picks the first time it is positioned.
...
When `lockPlacement` is true, the first `computePosition` still runs with `flip` and picks the side with more room. Every later tick then asks for that side directly, without `flip`. Scrolling and resizing still move the panel, but nothing can move it to the other side. `shift` and `size` still run, so the panel still shrinks when the space around it runs out.
...
`lockPlacement` defaults to false. Both behaviours only apply when it is true, so a caller that does not opt in is positioned exactly as before and gets no new attribute. It works in every component that already accepts an `anchor` config, including `Combobox`, `Listbox`, `Menu`, and `DatePicker`.
