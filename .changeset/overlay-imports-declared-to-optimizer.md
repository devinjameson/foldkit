...
A linked DevTools package, the workspace's own examples included, stays undeclared: the optimizer serves linked packages from source, and force-including one would freeze that source into the cache where edits to it no longer reach the page.
