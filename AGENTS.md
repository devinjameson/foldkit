...
- Use `Message.match<Update.Return<Model, Message>>` or a reused `UpdateReturn` alias for exhaustive Message matching. Match a `defineTaggedUnion` or `defineRouteUnion` value exhaustively through the union's own `match`, passing the return type as the generic when the branches need constraining: `UrlRequest.match<UpdateReturn>(request, { ... })`. Use Effect `Match` for partial matching, fallbacks, one handler shared across multiple tags, values narrowed to a subset of a union's variants, and unions without a `match` method (`taggedStruct` unions assembled by hand and factory-created unions such as `AsyncData`). For exhaustive Effect matches, prefer `M.tagsExhaustive({ ... })` over `M.tag(...)` chains. Never use `switch`.
