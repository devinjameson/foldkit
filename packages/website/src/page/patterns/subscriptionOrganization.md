...
| `Subscription.aggregate` | Combines two or more Subscriptions records into one, reading the Model, Message, and Effect services off the records. Throws at startup on duplicate keys instead of silently overriding.                           | A level combines multiple sources of Subscriptions (lifted children, inline entries, or both). |
