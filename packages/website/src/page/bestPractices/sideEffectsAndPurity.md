...
- [Subscription](/core/subscriptions) streams: a `Stream<Message>`. Subscriptions model ongoing processes like keyboard events, window resizing, or intersection observers. When an event must be cancelled before producing a Message, use `Subscription.fromEventPreventDefault`: its mapper runs inside the browser’s dispatch, where `event.preventDefault()` still takes effect, while a `Stream.mapEffect` stage runs on a later turn, after the browser has committed the default. The runtime controls when streams subscribe and unsubscribe based on your Model.
