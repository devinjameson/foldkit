...
Because the Stream factory is an Effect, it can do DOM side effects directly. When the work must be synchronous with the event itself, it has to run inside the listener callback: calling `preventDefault` on a keydown listener is the canonical case, since routing the event through `update` would arrive after the browser had committed the default. The same goes for downstream Stream operators. A callback passed to `Stream.mapEffect` or `Stream.tap` runs on a later turn than the dispatch, so a `preventDefault` there arrives after the default action and does nothing. The `Subscription.fromEvent` helpers below run their `toMessage` mapper inside the dispatch for exactly this reason, and `Subscription.fromEventPreventDefault` calls `preventDefault()` for you. `Stream.tap` remains the right shape for DOM mutations that do not need to land inside the dispatch and produce no Message.
...
The `toMessage` mapper runs synchronously in the same call stack as the browser’s event dispatch, so calling `event.preventDefault()` inside it takes effect, unless the listener is passive. Browsers register `wheel`, `mousewheel`, `touchstart`, and `touchmove` listeners as passive by default when the target is `window`, `document`, `document.documentElement`, or `document.body`, and a passive listener ignores `preventDefault()` and logs a console warning. Those are exactly the targets global Subscriptions listen on, so to cancel those events pass `options: { passive: false }` explicitly. Pass the `target` as a thunk when it may not exist until the scope opens, or directly for always-present globals like `window` and `document`. For events tied to a specific rendered element, reach for [Mount](/core/mount) instead, which hands you the element directly.
...
When a handled event should also cancel its default action, use `Subscription.fromEventPreventDefault`. Its `toMessage` returns `Option.some(message)` to mark the dispatch handled, and the runtime calls `event.preventDefault()` and emits the Message, both inside the browser’s dispatch; `Option.none()` leaves the default behavior intact. The mapper never calls `preventDefault()` itself. Because cancelling is the point, the helper registers the listener with `passive: false` when you did not specify it, so wheel and touch events on `window` and `document` stay cancelable, and it throws if you pass `passive: true` explicitly.
