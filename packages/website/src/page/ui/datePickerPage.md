...
DatePicker is a Submodel. Initialize it with `DatePicker.init()`, store its Model in the parent, wire its Messages through [`Update.foldChild`](/core/submodel#fold-child), and render it with `DatePicker.view()`.
