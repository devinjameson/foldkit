...
| `anchor`                | `AnchorConfig \| undefined`                              | —       | Floating positioning config: placement, gap, offset, padding, lockPlacement, and portal. The items panel is always anchored to the button; when omitted, the panel uses bottom-start placement. Portaled to the document body by default; pass portal: false to keep the panel inside the wrapper. |
