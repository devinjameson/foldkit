...
| `ClearedSelection` | `{}`              | —       | Emitted when a nullable combobox closes with an empty input, meaning the user cleared it. The parent clears the selection it owns. A read-only Combobox never emits it, so opening and closing one cannot clear the selection.                                                                                                                                                     |
