...
The natural follow-up question is: what if I need a field a typed handler does not expose? Today you cannot reach it through the DSL. The set of handlers is closed. In practice this is rarely the limit you hit, because the curated payloads cover the fields you usually want. The places it does bite are specialized: pen pressure on pointer events for drawing apps, `isComposing` on input events for IME-aware text editors, and multi-touch gesture data. Custom events are the exception: [CustomElement](/core/custom-element) declares an element's events with Schema and decodes each `detail` before it reaches your callback, so a third-party widget is already reachable with the same safety a typed handler gives you. For the rest, we intend to add an escape hatch shaped like Elm's `on`, taking a Schema that decodes the event and dispatches nothing when it fails. Until then, the set is what it is.
