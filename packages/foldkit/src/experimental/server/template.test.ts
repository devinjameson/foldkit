...
describe('injectIntoTemplate insertion context', () => {
  ...
  it('rejects a placeholder inside a form', () => {
    ...
    expect(() =>
      injectIntoTemplate(
        ...
        rendered({ html: '<form data-foldkit-app="app">hi</form>' }),
      ),
    ).toThrow(UNSUPPORTED)
  })
  ...
  it('accepts a placeholder in the second of two same-tag siblings', () => {
    ...
    const result = injectIntoTemplate(template, rendered())
    expect(result).toContain(
      '<section><div data-foldkit-app="app">hi</div></section>',
    )
  })

  it('does not validate against an earlier same-tag sibling', () => {
    // The earlier section holds a copy of what the injection should produce.
    // Resolving the path by tag name alone would find that copy and accept a
    // page whose real injection is somewhere else entirely.
    const template =
      '<!doctype html><html lang="en"><head><title>old</title></head>' +
      '<body><section><div data-foldkit-app="app">hi</div></section>' +
      '<section><div id="root"></div><p>tail</p></section></body></html>'

    const result = injectIntoTemplate(template, rendered())
    expect(result).toContain(
      '<section><div data-foldkit-app="app">hi</div><p>tail</p></section>',
    )
  })
  ...
  it('rejects a template whose placeholder is swallowed with scripting disabled', () => {
    ...
    expect(() =>
      injectIntoTemplate(
        template,
        rendered({ html: '<main data-foldkit-app="app">Application</main>' }),
      ),
    ...
    for (const attribute of ['shadowrootmode="open"', 'shadowroot="open"']) {
      expect(() =>
        injectIntoTemplate(
          ...
          rendered({
            html:
              '<div data-foldkit-app="app" id="host">' +
  ...
  it('accepts an ordinary template element in rendered markup', () => {
    const result = injectIntoTemplate(
      pageWithPlaceholderIn('', ''),
      rendered({
        html: '<div data-foldkit-app="app"><template><span>x</span></template></div>',
      }),
    )
  ...
  it('accepts a second injection beside an existing root with another runtime id', () => {
    // Two applications can share a document. The injection is identified by the
    // position its own placeholder held, so another application's root is not
    // mistaken for it and does not count against it.
    const template =
      '<!doctype html><html lang="en"><head><title>old</title></head>' +
      '<body><div data-foldkit-app="app-a">existing</div>' +
      '<div id="root"></div></body></html>'

    const result = injectIntoTemplate(
      template,
      rendered({ html: '<div data-foldkit-app="app-b">hi</div>' }),
    )

    expect(result).toContain('<div data-foldkit-app="app-a">existing</div>')
    expect(result).toContain('<div data-foldkit-app="app-b">hi</div>')
  })
})
