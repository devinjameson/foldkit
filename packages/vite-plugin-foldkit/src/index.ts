...
export type FoldkitPluginOptions = Readonly<{
   ...
   * renders each GET request that falls through Vite's own middleware by
   * loading `ssr.serverEntry` through the SSR module loader and calling its
   * `renderPage`. When `undefined` (the default), the dev server serves the
   * client entry only.
   */
...
export const foldkit = (options: FoldkitPluginOptions = {}): Array<Plugin> => {
  ...
  return options.ssr === undefined
    ? [foldkitViewIdentity(), hmrPlugin]
    : [foldkitViewIdentity(), hmrPlugin, foldkitSsr(options.ssr)]
}
