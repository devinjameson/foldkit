 ...
 *    focus between options; Space and clicking do nothing. No option is marked
 *    active: focus moves without dispatching, so `data-active` and `isActive`
 *    would otherwise keep pointing at the selection while focus sat elsewhere.
 *    Style focus with `:focus-visible` in this mode. The roving tab stop still
 *    sits on the selected option, since it derives from `selectedValue`, so
...
export const view = <Value extends string, Message>(
  ...
  const optionInfos: ReadonlyArray<OptionInfo<Value, Message>> = Array.map(
    ...
    (value, index) => {
      ...
      const isActive = isFocusable && !isReadOnly
