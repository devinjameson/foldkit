...
- Use `defineMessageUnion()` for Message unions, `defineTaggedUnion()` for other domain unions (Model states, field validation), and `defineRouteUnion()` for routes. Each declares the whole union from one record of fields per variant, and every variant is reached through the union: `Message.ClickedSubmit()`, `FetchState.Ok({ data })`, `AppRoute.Home()`. `taggedStruct()` is the escape hatch for a shape a single record cannot express, such as a union whose variants reference the union itself.
