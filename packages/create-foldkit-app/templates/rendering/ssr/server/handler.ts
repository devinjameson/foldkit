// NOTE: this declares `@types/node` for the whole project so the `node:path`
// and `node:url` imports below resolve. Under most package managers TypeScript
// discovers it on its own, because `@effect/platform-node` pulls it in through
// its own types. A Deno scaffold serves requests through
// `@effect/platform-deno` instead and loses that, so the reference is what
// keeps `tsc --noEmit` working there. A reference adds a type package rather
// than restricting the set, so adding your own global types needs no change
// here.
/// <reference types="node" />
import { Config, Effect, FileSystem, Match as M, Option } from 'effect'
import {
...
export const PORT = Config.withDefault(Config.port('PORT'), DEFAULT_PORT)
